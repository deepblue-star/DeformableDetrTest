...
class AttentionVisualizer:
    ...
    def compute_features(self, img):
        ...
        self.transformer_output_hs, self.transformer_output_init_reference, self.transformer_output_inter_references, _, _ = \
    ...
    def features_visualizaion(self):
        ...
        enc_attn_weights_1, enc_attn_weights_2, enc_attn_weights_3, enc_attn_weights_4 = torch.split(
    ...
    def get_nearest_point_query_id_of_bbox(self, reference_point, cx, cy):
        ...
        # 计算每个点到 (cx, cy) 的曼哈顿距离
        distances = np.abs(points[:, 0] - cx) + np.abs(points[:, 1] - cy)
    ...
    def enc_attention_visualizaion(self, path):
        ...
        bboxes = self.model_output['pred_boxes'][0, keep]
        cx, cy = bboxes[0][0], bboxes[0][1]
        query_id = self.get_nearest_point_query_id_of_bbox(reference_points[0, :, level_index, :], float(cx.cpu().data), float(cy.cpu().data))
        ...
        ax = plt.gca()
        plt.scatter(sampling_location_rescaled[:, 0], sampling_location_rescaled[:, 1], s=400,
                    c=viridis(100 * attention_weight))
        ...
        if all_inspect == True:
            ...
            if level_inspect == True:
                plt.savefig('output/visualization_enc_attention/query_' + str(query_id) + '_levels_' + str(
                    level_index) + '_heads_all_points_all.jpg')
                # plt.savefig('output/visualization_attention/cat_casade_6_enc_query_' + str(query_id))
            else:
                plt.savefig('output/visualization_enc_attention/query_' + str(query_id) + '_levels_all_heads_' + str(
                    head_index) + '_points_all.jpg')
    ...
    def enc_attention_visualizaion_v2(self, path, prob_threshold=0.97):
        ...
        keep = probas.max(-1).values > prob_threshold
        keep = keep.cpu().detach().numpy()
        valid_pred_ids = np.where(keep)

        heat_masks = []
        target_points = []
        print('got {} valid boxes'.format(len(valid_pred_ids[0])))
        for pred_id in list(valid_pred_ids[0]):
            heat_mask, target_point = self.get_one_valid_heatmap(
                path=path,
                pred_id=pred_id,
                ...
                enc_attn_sampling_offsets=enc_attn_sampling_offsets,
                enc_attn_weights=enc_attn_weights
            )
            heat_masks.append(heat_mask)
            target_points.append(target_point)
        if len(heat_masks) == 0:
            return None, None
        final_heat_mask = np.zeros_like(heat_masks[0])
        for heat_mask in heat_masks:
            final_heat_mask += heat_mask / len(heat_masks)
        # Normalize the heatmap to the range [0, 1] for visualization
        final_heat_mask = np.clip(final_heat_mask / np.max(final_heat_mask), 0, 1)

        coolwarm = cm.get_cmap('coolwarm', 100)
        final_heat_mask = coolwarm(1 - final_heat_mask)

        return final_heat_mask, target_points


    def get_one_valid_heatmap(self,
                              path,
                              pred_id,
                              reference_points,
                              level_index,
        ...
        attention_weight = attention_weight.cpu().detach().numpy()
        reference_point = reference_point.cpu().detach().numpy()
        return self.plot_enc_attn_img_with_heatmap(path, reference_point, sampling_location, attention_weight), query_point_coord

    def plot_enc_attn_img_with_heatmap(self, src_img_path, reference_point, sampling_location, attention_weight):
        self.pil_img = Image.open(src_img_path)
        ...
        heat_mask = self.generate_heatmask(sampling_location_rescaled, attention_weight, int(min(figsize)/50), figsize)

        return heat_mask

    def generate_heatmask(self, sampling_location_rescaled, attention_weight, sigma=5, img_size=(100, 100)):
        """
        ...
        """
        # Create an empty image to hold the heatmap values
        heatmap = np.zeros(img_size)

        # Create a grid of coordinates (X, Y) corresponding to the image size
        ...
        # 使用 tqdm 包装循环，显示进度条
        for i, (coord, weight) in tqdm(enumerate(zip(sampling_location_rescaled, attention_weight)),
                                       total=len(sampling_location_rescaled), desc="Generating heatmap"):
            # Calculate the distance from each pixel in the grid to the current point
            dist = cdist([coord], grid_coords).reshape(img_size)

            # Calculate the Gaussian influence at each point (higher weight near the point)
            gaussian = np.exp(-dist ** 2 / (2 * sigma ** 2))  # Gaussian kernel

            # Apply the weight to the Gaussian influence
            heatmap += weight[0] * gaussian
    ...
    def dec_attention_visualizaion(self, path):
        ...
        if all_inspect == True:
            ...
            if level_inspect == True:
                ...
                plt.savefig('output/visualization_dec_attention/query_' + str(query_id) + '_levels_all_heads_' + str(
                    head_index) + '_points_all.jpg')
    ...
    def reference_point_position_visualization(self, path):
        ...
        ax = plt.gca()
        plt.scatter(transformer_output_init_reference_rescaled[:, 0], transformer_output_init_reference_rescaled[:, 1],
                    s=20, c='white')
        ...
        for i in range(transformer_output_inter_references.shape[0]):
            ...
            transformer_output_inter_reference_rescaled = np.matmul(transformer_output_inter_reference[0, :, :],
                                                                    M_rescale)
            ...
            ax = plt.gca()
            plt.scatter(transformer_output_inter_reference_rescaled[:, 0],
                        transformer_output_inter_reference_rescaled[:, 1], s=20, c='white')
    ...
    def query_position_visualization(self, path):
        ...
        ax = plt.gca()
        plt.scatter(query_position_rescaled[:, 0], query_position_rescaled[:, 1], s=100 * query_end_size,
                    c=viridis(query_class_end_index))
        ...
        for i in range(len(query_inter)):
            ...
            ax = plt.gca()
            plt.scatter(query_position_inter_ith_rescaled[:, 0], query_position_inter_ith_rescaled[:, 1],
                        s=100 * query_inter_ith_size, c=viridis(query_class_inter_ith_index))
    ...
    def query_position_evolvement_visualization(self, path):
        ...
        ax = plt.gca()
        plt.scatter(query_position_rescaled[:, 0], query_position_rescaled[:, 1], s=50 * query_end_size,
                    c=viridis(query_class_end_index))
        # a = plt.scatter(query_position_rescaled[:, 0], query_position_rescaled[:, 1], s = 20, c =viridis(query_class_end_index))
        # fig.colorbar(a, ax=ax)
        ...
        for i in range(len(query_inter)):
            ...
            plt.scatter(query_position_inter_ith_rescaled[:, 0], query_position_inter_ith_rescaled[:, 1],
                        s=50 * query_inter_ith_size, c=viridis(query_class_inter_ith_index))

            if i != 0:
                if i == 4:
                    query_position_inter_ith_rescaled = query_position_rescaled
                    for n in range(query_position_inter_ith_rescaled.shape[0]):
                        plt.arrow(old_position[n][0], old_position[n][1],
                                  query_position_inter_ith_rescaled[n][0] - old_position[n][0],
                                  query_position_inter_ith_rescaled[n][1] - old_position[n][1],
                                  color=viridis(query_class_inter_ith_index[n]), head_width=2)
                else:
                    for n in range(query_position_inter_ith_rescaled.shape[0]):
    ...
    def reference_query_delta_visualization(self, path):
        ...
        for i in range(query_position_rescaled.shape[0]):
            plt.arrow(transformer_output_init_reference_rescaled[i, 0],
                      transformer_output_init_reference_rescaled[i, 1],
                      query_position_rescaled[i, 0] - transformer_output_init_reference_rescaled[i, 0],
                      query_position_rescaled[i, 1] - transformer_output_init_reference_rescaled[i, 1],
